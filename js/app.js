...
function showLoading() {
    ...
    loadingTimeout = setTimeout(() => { loadingCounter = 0; const el = document.getElementById('loading'); if (el) el.style.display = 'none'; }, 8000);
}
...
// ✅ FUNGSI BUILDER PESAN WHATSAPP (EKSPLISIT & AMAN)
function buildSingleMessage(a) {
    ...
    msg += "_Terima kasih._";
    console.log("📤 WA Message Generated:\n", msg); // Debug di Console
    return msg;
}
...
function buildBulkMessage(list) {
    let msg = "🏛️ *KEMENTERIAN AGAMA KAB. TANAH DATAR*\n";
    msg += "━━━━━━━━━━━━━━━━━━━━━━\n";
    msg += "📋 *AGENDA KEGIATAN PIMPINAN (TERPILIH: " + list.length + ")*\n\n";
    ...
    msg += "_Mohon kehadiran tepat waktu._\n";
    msg += "_Terima kasih._";
    console.log("📤 WA Bulk Message Generated:\n", msg); // Debug di Console
    return msg;
}
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="action-btns-compact">
                    ${rc !== 'admin' ? `
                    <button class="btn btn-danger btn-sm" onclick="forceLogoutUser('${u.username}')" title="Paksa Logout"><i class="fas fa-power-off"></i> Logout</button>
                    <button class="btn btn-success btn-sm" onclick="allowLoginUser('${u.username}')" title="Izinkan Login"><i class="fas fa-check"></i> Izinkan</button>` : `<span class="text-muted" style="font-size:0.8rem;">Admin Utama</span>`}
                </div>
            </td>
        `;
        tbody.appendChild(tr);
