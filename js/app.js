...
// ============================================
// AGENDA & WHATSAPP (FIXED HEADER & TEMPLATE)
// ============================================
...
function sendWhatsAppDirect(agenda) {
    ...
    // Header resmi tanpa logo gambar (WhatsApp text tidak support gambar)
    const msg = `🏛️ *KEMENTERIAN AGAMA KAB. TANAH DATAR*\n━━━━━━━━━━━━━━\n📅 *Tanggal:* ${tgl}\n⏰ *Pukul:* ${jam}\n📍 *Tempat:* ${tempat}\n\n📝 *Agenda Kegiatan:*\n${keg}\n\n📌 *Keterangan:*\n${ket}\n━━━━━━━━━━━━━━\n👤 *Input oleh:* ${author}\n_Mohon kehadiran tepat waktu. Terima kasih._`;
...
function openAgendaModal(id=null) {
    ...
    if(id) { const a=allAgenda.find(x=>x.id===id); if(!a)return showToast('Data tidak ditemukan','error'); document.getElementById('agendaId').value=a.id; document.getElementById('agendaTanggal').value=a.tanggal; document.getElementById('agendaWaktuMulai').value=a.waktu_mulai; document.getElementById('agendaWaktuSelesai').value=a.waktu_selesai; document.getElementById('agendaKegiatan').value=a.kegiatan; document.getElementById('agendaTempat').value=a.tempat; document.getElementById('agendaKeterangan').value=a.keterangan||''; }
}
...
// ============================================
// USERS (FIXED ROLE DISPLAY & MODAL)
// ============================================
async function loadUsers(){const res=await api('getUsers'); if(res.status==='success')renderUserTable(res.data);}
function renderUserTable(users){
    const tbody=document.querySelector('#usersTable tbody'); tbody.innerHTML=''; 
    if(!users.length){tbody.innerHTML='<tr><td colspan="6" class="text-center text-muted">Tidak ada user</td></tr>'; return;} 
    users.forEach((u,i)=>{
        // ✅ Sanitasi role agar selalu terbaca & badge sesuai
        ...
        const tr=document.createElement('tr'); 
        tr.innerHTML=`<td>${i+1}</td><td>${u.username}</td><td>${u.nama_lengkap}</td><td>${u.jabatan}</td><td><span class="badge ${badgeClass}">${roleLabel}</span></td><td><div class="action-btns"><button class="btn btn-warning btn-sm" onclick="editUser('${u.username}')"><i class="fas fa-edit"></i></button><button class="btn btn-danger btn-sm" onclick="deleteUser('${u.username}')" ${roleClean==='admin'?'disabled':''}><i class="fas fa-trash"></i></button></div></td>`; 
        tbody.appendChild(tr);
    });
}
    ...
    if(username){
        ...
        for(let row of rows){
            if(row.cells[1].textContent===username){
                ...
                // ✅ Ambil role bersih & set dropdown
...
async function handleUserSubmit(e){
    e.preventDefault(); const old=document.getElementById('userOldUsername').value; 
    const p={username:document.getElementById('userUsername').value, role:document.getElementById('userRole').value, nama_lengkap:document.getElementById('userNama').value, jabatan:document.getElementById('userJabatan').value}; 
    const pwd=document.getElementById('userPassword').value; let res; 
    if(old){if(pwd)p.newPassword=pwd; res=await api('updateUser',p);}else{p.password=pwd; res=await api('addUser',p);} 
...
// ============================================
// DASHBOARD, CALENDAR, IMPORT/EXPORT, SETTINGS
// ============================================
